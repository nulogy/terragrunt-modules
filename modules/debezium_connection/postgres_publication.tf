resource "null_resource" "postgres_publication" {
  triggers = {
    ...
    granter_password = var.database_admin_password
    granter_username = var.database_admin_username
  ...
  provisioner "local-exec" {
    command = <<EOF
docker run -e PGPASSWORD="${self.triggers.granter_password}" --rm --entrypoint="" postgres:${self.triggers.postgres_version}-alpine \
  psql \
  --host ${self.triggers.database_address} \
  --port ${self.triggers.database_port} \
  --username ${self.triggers.granter_username} \
