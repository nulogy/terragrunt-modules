locals {
  normalized_service_name = replace(var.kafka_connect__service_name, "_", "-")
  ...
  envars = jsonencode([
    ...
    // The topic must have a single partition, should be highly replicated (e.g., 3x or more) and should be configured for compaction
    {
      ...
      value = "kconnect-config-${local.normalized_service_name}"
    },
    {
    ...
    // The topic should have many partitions, be highly replicated (e.g., 3x or more) and should be configured for compaction
    {
      ...
      value = "kconnect-offset-${local.normalized_service_name}"
    },
    {
    ...
    // The topic can have multiple partitions, should be highly replicated (e.g., 3x or more) and should be configured for compaction
    {
      ...
      value = "kconnect-status-${local.normalized_service_name}"
    },
    {
    ...
    {
      name  = "GROUP_ID",
      value = local.normalized_service_name
    },
    {
      name  = "ENVIRONMENT",
      value = local.normalized_service_name
...
module "kafka_connect" {
  ...
  ecs_service_name          = local.normalized_service_name
  ecs_incoming_allowed_cidr = module.vpc.vpc_cidr
  envars                    = local.envars
  environment_name          = local.normalized_service_name
  ...
  param_store_namespace     = local.normalized_service_name
  private_subnets           = module.vpc.private_subnet_ids
  public_subnets            = module.vpc.public_subnet_ids
  security_group_name       = "${local.normalized_service_name}-kafka-connect-sg"
  service_name              = "${local.normalized_service_name}-kafka-connect"
  slow_start                = 30
