...
docker run -e PGPASSWORD="${data.aws_ssm_parameter.db_password.value}" --rm --entrypoint="" postgres:${var.database_engine_version}-alpine \
  ...
  --command "CREATE ROLE ${local.db_debezium_user_name} with LOGIN PASSWORD
    '${data.aws_ssm_parameter.debezium_db_password.value}' VALID UNTIL 'infinity' IN ROLE rds_replication;
    GRANT USAGE ON SCHEMA public TO debezium;
    GRANT SELECT, INSERT, DELETE ON TABLE ${var.debezium_config__events_table} TO ${local.db_debezium_user_name};
...
docker run -e PGPASSWORD="${data.aws_ssm_parameter.db_password.value}" --rm --entrypoint="" postgres:${var.database_engine_version}-alpine \
  ...
  --username ${var.database_user} \
  --dbname "${var.database_name}" \
  --command "REVOKE ALL PRIVILEGES ON TABLE ${var.debezium_config__events_table} FROM ${local.db_debezium_user_name};
    REVOKE USAGE ON SCHEMA public FROM debezium;
    REVOKE CREATE ON DATABASE ${var.database_name} FROM ${local.db_debezium_user_name};
    DROP ROLE ${local.db_debezium_user_name};"
EOF
}

resource "null_resource" "create_debezium_user" {
  triggers = {
    role           = local.db_debezium_user_name
